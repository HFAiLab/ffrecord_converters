...
def preprocess_map(vectors, patch_size, canvas_size, num_classes, thickness, angle_class):
    ...
    instance_gt = np.stack(instance_gt)
    forward_masks = np.stack(forward_masks)
    backward_masks = np.stack(backward_masks)

    instance_gt = overlap_filter(instance_gt, filter_masks)
    ...
    return torch.tensor(instance_gt), torch.tensor(forward_masks), torch.tensor(backward_masks)
